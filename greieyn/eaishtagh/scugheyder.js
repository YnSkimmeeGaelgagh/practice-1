...
function scugh () {
    ...
    function curFockleStiagh (f) {
        ...
        f.onpointerdown = () => {
            cloieSheean({raad: sheeanArraghey});
            f.remove();
    ...
    crammanFreggyrt.onclick = t => {
        ...
        function freggyrtKiart () {
            ...
            if (pointRyGheddyn) {
                agg++;
                raaghyn[earrooFeysht].focklyn.forEach(f => coEarrooagheyAggNoa(f, 2, 1));
            };
            earrooFeysht++;
            kiedHuittym = true;
